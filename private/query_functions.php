...
function validate_admin($admin) {
  ...
  if (!is_int($admin['id'])) {
    $errors[] = "The id is not an integer.";
  }
  ...
  if(is_blank($admin['email'])) {
    ...
    } elseif(!has_length($admin['email'], ['min' => 5, 'max' => 255])) {
      $errors[] = "Email must be between 5 and 255 characters.";
    }
...
function insert_admin($admin) {
  ...
  $sql .= "(id, first_name, last_name, email, username) ";
  $sql .= "VALUES (";
  $sql .= "'" . db_escape($db, $id) . "', ";
